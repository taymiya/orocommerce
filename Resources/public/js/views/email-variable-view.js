    ...
    EmailVariableView = BaseView.extend({
        ...
        _getEntityVariablesHtml: function (variables, entityLabel, path, pathLabels) {
            ...
            _.each(variables, function (variable, varName) {
                ...
                if (varName == "subtemplate") {
                    var subVariable =  JSON.stringify(variable);
                    var parsed = JSON.parse(subVariable);
                     _.each(parsed, function (variable , id) {
                      subTemplates[id] = variable;
                    });
                }

            });
            //alert(JSON.stringify(fields, null, 4));
            //alert(JSON.stringify(subTemplates, null, 4));
