...
class PDFTemplateController extends RestController
{
     ...
     * @Get("/pdftemplates/variables/{entityName}/{custom}"
     * )
     *
     * @return Response
     */
    public function getVariablesAction($entityName = null , $custom = '0')
    {
        ...
        $result = $provider->getEntityVariableDefinitions(null,$entityName);
        
        if($custom == '1' ){
           if(isset($result[$entityName])){
               $add = $provider->getEntityVariableDefinitionsAdditional(null,$entityName);
               if(count($add)>0){
                   foreach($add as $key => $ad){
                       $result[$entityName][$key] = $ad;
                   }
               }
